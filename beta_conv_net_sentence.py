...
def store_sent(sentence, num, datasets):
    p_sento_finale = []
    sento_finale = []
    print "sentence {0} len {1}".format(num, len(sentence))
    for ind in xrange(0,len(sentence[0])):
        if len(datasets[0]) > ind:
            off = []
            p_off = []
            off = np.append(sentence[0][ind],datasets[0][ind,-1])
            p_off = np.append(sentence[0][ind],(datasets[0][ind,-1]))
